...
class BADataset():
    def __init__(self, path, L, is_train, is_valid, is_test):
        ...
        for n in range(len(self.label)):
            ...
            indic = np.array(sorted(graph.edges)) # (nnz/2, 2) (無向グラフ：[0, 1]と[1, 0]を区別しない)
            ...
            # mask をスパース化する
            indic_mask = torch.Tensor(self.mask[n]).to_sparse()._indices().numpy().transpose(1, 0)  # (nnz, 2) (有向グラフ：[0, 1]と[1, 0]が存在)
            graph = nx.Graph()
            graph.add_edges_from(indic_mask.tolist())
            indic_mask = np.array(sorted(graph.edges))  # (nnz/2, 2) (無向グラフ：[0, 1]と[1, 0]を区別しない)
            indic_mask = np.array([sorted(edge) for edge in indic_mask])  # (existing_node, new_node)の順序
